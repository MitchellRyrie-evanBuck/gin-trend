...
import (
	...
	"github.com/afl-lxw/gin-trend/initialize/internal"
...
func (m *mongo) Initialization() error {
	...
	if global.TREND_CONFIG.Mongo.IsZap {
		opts = internal.Mongo.GetClientOptions()
	}
	ctx := context.Background()
	client, err := qmgo.Open(ctx, &qmgo.Config{
		Uri:              global.TREND_CONFIG.Mongo.Uri(),
		Coll:             global.TREND_CONFIG.Mongo.Coll,
		Database:         global.TREND_CONFIG.Mongo.Database,
		MinPoolSize:      &global.TREND_CONFIG.Mongo.MinPoolSize,
		MaxPoolSize:      &global.TREND_CONFIG.Mongo.MaxPoolSize,
		SocketTimeoutMS:  &global.TREND_CONFIG.Mongo.SocketTimeoutMs,
		ConnectTimeoutMS: &global.TREND_CONFIG.Mongo.ConnectTimeoutMs,
		Auth: &qmgo.Credential{
			Username: global.TREND_CONFIG.Mongo.Username,
			Password: global.TREND_CONFIG.Mongo.Password,
		},
	}, opts...)
	if err != nil {
		return errors.Wrap(err, "链接mongodb数据库失败!")
	}
	global.TREND_MONGO = client
	err = m.Indexes(ctx)
	if err != nil {
		return err
	}
...
func (m *mongo) CreateIndexes(ctx context.Context, name string, indexes [][]string) error {
	collection, err := global.TREND_MONGO.Database.Collection(name).CloneCollection()
	if err != nil {
	...
	for k1, v1 := range indexMap1 {
		...
		if len(fmt.Sprintf("%s.%s.$%s", collection.Name(), name, v1)) > 127 {
			err = global.TREND_MONGO.Database.Collection(name).CreateOneIndex(ctx, options.IndexModel{
				Key:          v1,
